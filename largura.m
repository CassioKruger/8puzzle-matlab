...
%                 D_anterior = D_atual;
                ...
                [L,R,U,D]=mov_largura(E_atual); %Procura vizinhos                        
                [L,R,U,D]=teste_ciclo_largura(L,R,U,D,aberto); %testa se algum movimento é repetido
                [obj_atingido, mov_final] = checkobj_largura(L,R,U,D,Ef); %confere se algum nó vizinho é o objetivo             
                fechados = vizinhos_append_largura(L,R,U,D,fechados,D_atual,E_atual); %Coloca eles na fila pra serem explorados
                if(obj_atingido)
                    aberto(pos+1).estado = mov_final;
                    aberto(pos+1).custo = D_atual+1;
                    aberto(pos+1).pai = E_atual;
                    aberto(1) = [];
                    passos = caminho_largura(aberto,E0,Ef);
                toc
                end
                if(isempty(fechados))
                    obj_atingido=1;
